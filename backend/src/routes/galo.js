...
router.get('/schedule', (req, res) => {
  const row = db.prepare('SELECT * FROM galo_schedule WHERE family_id = ?').get(req.user.familyId);
...
router.get('/news-state', (req, res) => {
  const row = db.prepare('SELECT * FROM galo_news_state WHERE family_id = ?').get(req.user.familyId);
